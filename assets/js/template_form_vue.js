...
createApp({
    ...
    {{/if}}`,
        ...
        const isEdit = ref(IS_EDIT);
        const submitUrl = isEdit.value
            ? `${siteUrl.value}templates/edit/${TEMPLATE_DATA.id}`
            : `${siteUrl.value}templates/create`;
        ...
        onMounted(() => {
            ...
            htmlEditor.setValue(form.value.html_pattern, -1);
            htmlEditor.on("change", () => {
                form.value.html_pattern = htmlEditor.getValue();
            });
            ...
            jsonEditor.setValue(form.value.form_config, -1);
            jsonEditor.on("change", () => {
                form.value.form_config = jsonEditor.getValue();
            });
        });
        ...
        const saveTemplate = async () => {
            // Validate JSON
            try {
                JSON.parse(form.value.form_config);
            } catch (e) {
                alert("Invalid JSON Config: " + e.message);
                return;
            }

            // Submit Form (Standard POST for CodeIgniter)
